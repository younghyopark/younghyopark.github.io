...
def _update_network(self):
  ...
  if self.args.cuda:
    ...
    with torch.no_grad():
      ...
      target_q_value = r_tensor + self.args.gamma * q_next_value   # This is now a n-dimensional vector (as a batch)
...
- **DDPG + HER (Sparse reward)**
  ...
  Defining goal state (position) as $\mathbf{g}=[g_x, g_y, g_z]$ and current state (object position) as $\mathbf{s} = [s_x, s_y, s_z]$, original DDPG algorithm uses the reward with predefined threshold $t$.  Trained the network for 50 epochs, 50 samples collected per epoch, updated the network 40 times per epoch.
...
- **DDPG + HER + MultiRL**
  ...
  For my version of DDPG, I used a vector-format reward function which each component weighs the distance difference in each axis differently with $$M=10$$
...
Same thing happens for the simplest reach task as well. While the baseline model trained with a sparse reward stops trying to do better when the gripper reaches near certain boundary around the goal position, the model trained with multi reward for training constantly tries to precisely align the gripper right near the goal position.

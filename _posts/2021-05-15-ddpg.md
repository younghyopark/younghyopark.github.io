...
- **Distance based Vector Reward** 
    ...
    Of course, there might be **many more possible vector type reward functions**, which we will go through in the later part of the research. Before that, we first have to check whether our DDPG + multiRL algorithm indeed does a decent job of training the model. Doing so, we will use the vector reward defined above. 
